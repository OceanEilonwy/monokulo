//! Test-only helper for standing up a real, network-bound `scanner`
...
//! `scanner-test-support -> scanner -> shared`. That's not a
...
///
/// This is deliberately *not* `scanner::daemon::fake::FakeDaemonClient`: that
/// type is `#[cfg(test)]`-gated inside `scanner` itself, which means it is
...
/// A running, real (network-bound) `scanner` engine instance started by
...
impl TestEngineConfig {
    ...
    /// Boots a real `scanner` engine - in-memory `Store`,
...
/// Boots a real `scanner` engine with no configured Monero networks -
...
mod tests {
    ...
    fn temp_socket_path() -> std::path::PathBuf {
        ...
        path.push(format!(
            "scanner-test-support-key-custody-{}-{n}.sock",
            std::process::id()
        ));
        path
    }
