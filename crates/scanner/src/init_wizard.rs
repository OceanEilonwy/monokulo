//! `scanner --init`: an interactive wizard that produces (or updates) a
...
/// one) depending on where `scanner` happened to be run from, and so would
...
impl WizardAnswers {
    ...
    pub fn render_toml(&self) -> String {
        ...
        out.push_str(
            "# scanner configuration - generated by `scanner --init`.\n\
             #\n\
             ...
             # line to change it. Re-run `scanner --init` (optionally with --stagenet\n\
...
pub async fn run_interactive<R: BufRead, W: Write>(
    ...
    if answers.wallet.is_some() {
        writeln!(output, "  1. Start the server:   scanner{config_flag}")?;
        writeln!(
            output,
        ...
        writeln!(output, "     scanner --snippet{config_flag}")?;
    } else {
        writeln!(output, "  1. Start the server:   scanner{config_flag}")?;
        writeln!(
            output,
        ...
        writeln!(output, "     scanner --snippet{config_flag} --pk <pk_the tenant you just created>")?;
    }
    writeln!(
        output,
