...
//!
//! Moved here from `scanner`'s own `src/network.rs` alongside
...
//! testnet string mapping `scanner` already uses everywhere else, and
...
//! silently meaning something different in two places). `scanner`'s
