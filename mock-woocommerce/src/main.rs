...
//! against a real, already-running control-plane instance and prints the
...
//!
//! No config-file/CLI-parsing system needed at this scale: the control-plane
...
async fn main() -> ExitCode {
    let control_plane_base_url = std::env::args()
        .nth(1)
        .or_else(|| std::env::var("MOCK_WOOCOMMERCE_CONTROL_PLANE_URL").ok())
        .unwrap_or_else(|| DEFAULT_CONTROL_PLANE_BASE_URL.to_string());

    match mock_woocommerce::run_connect_flow(&control_plane_base_url).await {
        Ok(credentials) => {
            println!("connect flow succeeded against {control_plane_base_url}");
        ...
        Err(err) => {
            eprintln!("connect flow failed against {control_plane_base_url}: {err}");
            ExitCode::FAILURE
        }
    }
}
