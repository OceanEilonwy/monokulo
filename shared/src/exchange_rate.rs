...
//!
//! These are deliberately *not* behind one shared trait - control-plane
//! picks between them per-order (`control_plane::exchange_rate_config::
...
/// check buried in the dispatcher) so control-plane's per-order provider
...
impl XmrIdentityProvider {
    ...
    /// site - control-plane's dispatcher can `.await` either uniformly.
    /// Always exactly `["XMR"]`.
...
impl CoingeckoRateProvider {
    ...
    /// about how long a rate should be trusted for - control-plane's own
    ...
    /// to a stale cached value - the caller (control-plane's own
    ...
    /// lookup uses. Exists so control-plane can offer/validate currencies
