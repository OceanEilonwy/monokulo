...
//! component (e.g. the control-plane database) can reuse the same
...
//! guards against. The engine (`moneropay-core`) keeps its own `MIGRATIONS`
...
pub fn apply(conn: &Connection, migrations: &[(i64, &str)]) -> rusqlite::Result<()> {
    ...
    for (version, sql) in migrations {
        ...
        if !already_applied {
            ...
            // `moneropay_core::store`'s single-writer discipline for why the
