...
//!
//! `PlainKeyCustody` (`moneropay-core`'s `src/key_custody/plain.rs`) is the only
...
//!
//! ## Why this trait lives in `shared`, not `moneropay-core`, as of WBS 2.1.3
//!
//! Every other engine type lives in `moneropay-core` (`src/key_custody/mod.rs` used
//! to define all of this directly) — `shared` only ever held logic genuinely common
//! to the engine and the control-plane (secret-token hashing, HMAC signing,
...
//! structural reason, not a style one: `moneropay-core`'s own `main.rs` needs to be
...
//! itself - so as long as those types were defined inside `moneropay-core`,
//! `key-custody-service` had to depend on `moneropay-core` to reach them (true since
//! WBS 2.1.1). Once `main.rs` (part of the `moneropay-core` package) also needs to
//! depend on `key-custody-service` for `SocketKeyCustody`, that becomes
//! `moneropay-core -> key-custody-service -> moneropay-core` - a real, hard cycle
...
//! (which nothing in this cycle needs to depend on `moneropay-core` to reach) breaks
//! it: `key-custody-service` now depends on `shared` for these types instead of
//! `moneropay-core`, `moneropay-core` re-exports them from `shared` so every existing
//! `moneropay_core::key_custody::{KeyCustody, WalletHandle, ...}` import in the
//! engine keeps compiling completely unchanged (a `pub use` re-export is the same
//! type, not a wrapper - nothing downstream of `moneropay-core::key_custody` needed
...
//! where it was (`moneropay-core`'s own `src/key_custody/plain.rs`) - only the
...
//! real `PlainKeyCustody`) still needs `moneropay-core` - there is no way around
...
//! two separate crates (`key-custody-server` depends on both `moneropay-core` and
//! `key-custody-service`; `key-custody-service` itself depends on neither
//! `moneropay-core` nor `key-custody-server`). See `docs/WOOCOMMERCE_WBS.md`'s
...
impl WalletHandle {
    ...
    /// now (`moneropay-core`'s `PlainKeyCustody`, `key-custody-service`'s
