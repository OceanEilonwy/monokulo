...
//! `moneropay_core::key_custody::PlainKeyCustody` and answering every call over a
...
//! of WBS 2.1.3, once `moneropay-core`'s own `main.rs` needed to depend on
//! `key-custody-service` for `SocketKeyCustody` - this crate's `server.rs` needs a
//! real `PlainKeyCustody`, which only exists in `moneropay-core`, so keeping it in
//! the same crate `main.rs` depends on would have made `moneropay-core` depend on
//! a crate that depends on `moneropay-core`, a real Cargo dependency cycle. See
//! `server.rs`'s own module doc comment, and `shared/src/key_custody.rs`'s, for
//! the full account.
//!
//! This crate depends on both `moneropay-core` (for `PlainKeyCustody`) and
