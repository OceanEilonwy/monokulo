 ...
 * monokulo phpunit`) with the original naming, even though the class
...
/**
 * Asserts `WC_Gateway_Monokulo` is a real, registered WooCommerce payment
 ...
 * acceptable end state for this step ("Monero (via Monokulo)" showing
 * up, "disabled state is fine at this step") - and `WC_Gateway_Monokulo`
 * genuinely ships disabled by default (see the constructor's own comment on
 * `$this->enabled` in `includes/class-wc-gateway-monokulo.php`), since there
...
class GatewayRegistrationTest extends WP_UnitTestCase {
	 ...
	 * rather than repeating the literal string `'monokulo'` a second
	 * time in this file, so a future rename of the constant in
	 * `class-wc-gateway-monokulo.php` can't silently desync from what this
	 ...
	 * knows about `WC_Gateway_Monokulo` - proving the
	 * `woocommerce_payment_gateways` filter hook in `monokulo.php`
	 ...
	 * isolation (a test that only did `new WC_Gateway_Monokulo()` would pass
	...
	public function test_gateway_is_registered_with_woocommerce() {
		...
		// `monokulo_add_gateway_class()` callback) at that point.
		...
		$this->assertContains(
			$expected_id,
			$registered_ids,
			'Expected WC_Gateway_Monokulo (id "' . $expected_id . '") to appear in ' .
			...
			'means either the plugin\'s add_filter() call in monokulo.php never ran, ' .
		...
		$this->assertInstanceOf(
			...
			'WC_Gateway_Monokulo instance, not merely an id string collision.'
		);
	}
	...
	public function test_gateway_is_disabled_by_default() {
		...
		$this->assertSame(
			'no',
			$gateway->enabled,
			'WC_Gateway_Monokulo should ship disabled by default (WBS 1.5.1\'s own stated ' .
