 ...
 * class itself splits them (see `WC_Gateway_Monokulo::handle_connect_return()`'s
 * own doc comment for why):
 *
 * - `generate_monokulo_connect_html()`: the settings-screen button really
 ...
 *   `admin_post_monokulo_connect_return` action (`handle_connect_return()`)
...
class ConnectFlowTest extends WP_UnitTestCase {
	 ...
	 * at, via the `monokulo_control_plane_base_url` filter
	 * `WC_Gateway_Monokulo::get_control_plane_base_url()` exposes - a fake,
	 * obviously-non-resolving host, not `WC_Gateway_Monokulo::
	...
	public function set_up(): void {
		...
		add_filter( 'monokulo_control_plane_base_url', array( $this, 'filter_control_plane_base_url' ) );
	}

	public function tear_down(): void {
		remove_all_filters( 'pre_http_request' );
		remove_filter( 'monokulo_control_plane_base_url', array( $this, 'filter_control_plane_base_url' ) );
		parent::tear_down();
	}
	...
	private function render_connect_button_and_capture_nonce() {
		...
		$html        = $gateway->generate_monokulo_connect_html( 'connect', $field_data );
	...
	public function test_connect_button_links_to_the_real_connect_start_url_with_a_stored_nonce() {
		...
		$this->assertStringContainsString(
			'return_url=' . rawurlencode( admin_url( 'admin-post.php?action=monokulo_connect_return' ) ),
			$html,
		...
		$this->assertSame(
			$nonce,
			get_transient( 'monokulo_connect_nonce_monokulo' ),
	...
	public function test_process_connect_return_saves_settings_and_enables_the_gateway_on_success() {
		...
		$callback_gateway = new WC_Gateway_Monokulo();
		...
		$this->assertStringContainsString( 'section=monokulo', $redirect_url );
		$this->assertStringContainsString( 'monokulo_connected=1', $redirect_url );
	...
	public function test_process_connect_return_rejects_a_mismatched_nonce_without_calling_finish() {
		...
		$this->assertStringContainsString( 'monokulo_connect_error=nonce', $redirect_url );
		$this->assertNull(
		...
		$this->assertSame(
			'',
			( new WC_Gateway_Monokulo() )->get_option( 'public_key' ),
	...
	public function test_process_connect_return_rejects_reusing_the_same_nonce_twice() {
		...
		$first_gateway = new WC_Gateway_Monokulo();
		...
		$second_gateway = new WC_Gateway_Monokulo();
		...
		$this->assertStringContainsString( 'monokulo_connect_error=nonce', $second_result );
