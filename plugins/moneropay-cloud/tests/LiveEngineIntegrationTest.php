 ...
 * running `moneropay-core` engine - not `pre_http_request`-mocked
 ...
 *    # Config::validate()):
 *    mkdir -p /tmp/moneropay-engine-test
 *    # ... write /tmp/moneropay-engine-test/config.toml - see
 *    # work_notes.md's WBS 1.5.2 entry for the exact contents used here,
 *    # bind = "0.0.0.0:8180".
 *
 *    docker run -d --name moneropay-engine-test \
 *      --network wp-env-moneropay-cloud-9652ae59_default \
 *      -v "$(pwd)/target/release/moneropay-core:/moneropay-core:ro" \
 *      -v /tmp/moneropay-engine-test:/cfg -w /cfg \
 *      archlinux:latest /moneropay-core /cfg/config.toml
 *    ```
 *
 ...
 *
 *    The network name (`wp-env-moneropay-cloud-9652ae59_default`) is
 ...
 *
 *    ```json
 *    { "endpoint": "http://moneropay-engine-test:8180", "public_key": "pk_..." }
 *    ```
 *
 *    `moneropay-engine-test` (the container *name*) is what resolves here,
...
class LiveEngineIntegrationTest extends WP_UnitTestCase {
	...
	public function set_up(): void {
		...
		if ( ! file_exists( $this->live_config_path() ) ) {
			$this->markTestSkipped(
				'No tests/live-engine.local.json found - this test needs a real, running moneropay-core ' .
	...
	public function test_process_payment_creates_a_real_engine_order_and_redirects_to_it() {
		...
		$gateway = new WC_Gateway_MoneroPay(); // Re-read back from wp_options, like a real request would.
		...
		$this->assertSame(
			$payment_id,
			$order->get_meta( '_moneropay_cloud_payment_id' ),
